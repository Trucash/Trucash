...
pub fn init_chain() -> Result<bool, SuperError> {
	...
	database::write_chain_params_db(vec![0,2], prev_hash.to_vec());

	/// Initial utxo
	let intial_receiver_raw_address = [184, 247, 169, 32, 15, 18, 89, 62, 65, 113, 126, 100, 21, 109, 138, 129, 183, 137, 114, 52, 90, 50, 88, 201, 40, 198, 174, 64, 41, 209, 48, 127];
	let intial_receiver: CompressedRistretto = CompressedRistretto::from_slice(&intial_receiver_raw_address);
...
pub fn init_utxo(receiver: CompressedRistretto) -> Result<bool, SuperError> {
	...
	let mut diffie_hellman_key = crypto::create_diffie_hellman(private_key, receiver);
	...
	utxo.append(&mut receiver.to_bytes().to_vec());
	...
	utxo.append(&mut public_key.to_bytes().to_vec());
